#!/usr/bin/env python3
"""run_cycle.py - authoritative cycle logic using exact_divisor_count.

Writes ONLY clean JSON to artifacts/<CYCLE_ID>/advance-report.json
"""
import json
import os
import sys
...
def main():
    ...
    scratch = Path(os.environ.get("SCRATCH", str(pgs / "scripts" / "pgs-research-director" / "artifacts")))
    cycle_id = os.environ.get("CYCLE_ID", "cycle-" + datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S"))
    art_dir = scratch / cycle_id
    art_dir.mkdir(parents=True, exist_ok=True)

    # Parse for job info (requirement)
    job_id = None
    try:
    ...
    report = {
        ...
        "gwr_23_29": gwr(23, 29),
        "gwr_89_97": gwr(89, 97),
        "referenced": ["research/00-index/continuity/hourly_current_job.json", "AGENTS.md", "PROOF.md"],
        "status": "ADVANCE",
        "next_step": "Continue with parsed job or next tranche."
    }
