...
2. **`research/pgs-rh-placement-empirics-2026-06/d4_fractional_position_bound.md`** — d=4 fractional-position lemmas (RH-030–032).
3. **`lean-4/PGS/Placement.lean`** + **`lean-4/PGS/ChamberReset.lean`** — partial machine-checked mirror.

This roadmap orders the **minimal closure DAG** to move RH-080/081 from partial scaffold to a faithful end-to-end Lean mirror of the proved placement layer, without conflating it with RH-corpus Q1–Q5 (pole placement, transfer lemma, deconvolution).
...
| `gwr_d4_*` (Phases 1–5) | `Placement.lean` | `d4_fractional_position_bound.md` | 12+ corollaries machine-checked |
...
|------|-----------------|--------|--------------|---------|
| `sorry` | `three_distinct_divisors_imply_tau_ge_three` | `Basic.lean` | `PROOF.md` L80–81 | Pure-List counting; deferred Phase 2 Mathlib |
| `sorry` | `tau_eq_two_iff` reverse length step | `Basic.lean` | `PROOF.md` L80–81 | Symmetric counting deferral |
| `sorry` | `tau_gt_two_iff_has_proper_divisor` | `Basic.lean` | `PROOF.md` L80–81 | Depends on counting lemmas |
...
M3 is largely complete (RH-080). M5–M6 extend beyond placement into full `PROOF.md` stack.
...
|----|-------------|----------|----------|---------|
| M1.1 | `three_distinct_divisors_imply_tau_ge_three` | L80–81 | P0 | — |
| M1.2 | `tau_eq_two_iff` reverse length | L80–81 | P0 | M1.1 |
| M1.3 | `tau_gt_two_iff_has_proper_divisor` | L80–81 | P1 | M1.1, M1.2 |
| M1.4 | `prime_iff_tau_eq_two` (exported) | L80–81 | P1 | M1.1–M1.3 |
...
|----|-------------|--------------|--------|
| M3.1 | `gwr_d4_frac_pos_bound` and Phase 1–5 lemmas | `d4_fractional_position_bound.md` §2–6 | **Proved** |
...
**Gate:** `pgs-rh-placement-invariants.lean` smoke `#check` lines remain green; RH-080 boundary reads "Phases 1–5 proved; axioms M2 pending."
...
| M4.2 | Modulus-link density / tiling contradiction | §Prime-Square Proximity (651–666); shortcomings S1 | P0 | M4.1, M2.2 |
...
| ID | Lean target | PROOF.md | Priority | Depends |
|----|-------------|----------|----------|---------|
| M5.1 | `ordered_comparison_lemma` | §Ordered Comparison Lemma (199–224) | P1 | M1, `Placement.E/F` |
| M5.2 | `interior_maximizer_theorem` | §Interior Maximizer (186–198) | P1 | M5.1 |
| M5.3 | `finite_bounded_compression_base` | §Finite Bounded-Compression (546–587) | P2 | audit certificate import |
...
|------------------|----------------|--------------------------|-----------|---------------|
| Basic Objects — τ, prime ↔ τ=2 | 80–81, 134–145 | `PGS/Basic.lean` | M1 | Partial (`sorry`) |
| Interior Maximizer Theorem | 186–198 | `PGS/GWR.lean` (planned) | M5 | Not started |
| Ordered Comparison Lemma | 199–224 | `PGS/GWR.lean` (planned) | M5 | Not started |
| Witness Threshold Lemma | 315–358 | `Placement.witnessThreshold_*` | M3 | **Proved** |
| Prime-Square Case | 287–314 | `Placement.tau_prime_square_eq_three` | M2 | Axiom |
| Large-Divisor Adjacent Closure | 404–531 | `Placement.gwr_d4_closure_*` | M3 | **Proved** |
| Finite Bounded-Compression Base | 546–587 | (planned `BoundedCompression.lean`) | M5 | Not started |
| Prime-Square Proximity Theorem | 584–637 | `ChamberReset.near_root_*`, `prime_square_*` | M4 | Algebraic proved; main theorem trivial |
| Universal bounded compression (headline) | 40–44 | (planned) | M5 | Not started |
...
| RH-004 | M4 | Close M4.2–M4.3 |
| RH-080 | M0–M3 | Phases 1–5 proved; track M2 axioms |
