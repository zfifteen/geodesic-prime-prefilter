...
consecutive returns. The arithmetic structure inside prime gaps proves the
Riemann Hypothesis because it shows the exact integer-level pattern that the
zeta function later records in compressed form.
...
The Riemann Hypothesis is the classical pole-language description of the same
...
compressed wording of that source structure.

- Explanatory status: PGS supplies the upstream divisor-count structure whose
  analytic compression produces the RH language.
...
- Formal artifact status: `PROOF.md` proves the local next-prime and
  ...
  downstream zeta-language description, not the object that defines PGS.
...
- RH records a spectral-side collapse on the analytic continuation of the
...
divisor structure, while RH is the zeta-language statement of the same source
after analytic compression.
...
This note records why those objects produce the zeta-language statement called
...
- RH is the zero-geometry language of the analytic detector recovered from that
