...
def audit_case(
    ...
    public_structure_found = public_lower is not None or public_upper is not None
    factor_found = False
    if integrity_pass and public_structure_found:
        inferred = {
            str(gmpy2.mpz(str(value)))
            for value in (public_lower, public_upper)
            if value is not None
        }
        factor_found = bool(inferred & {str(p_value), str(q_value)})
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--inference",
        type=Path,
        default=THIS_DIR / "output" / "inference_rows.jsonl",
        help="Inference rows JSONL path.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=THIS_DIR / "output" / "audit_results.csv",
        help="Audit CSV output path.",
    )
    parser.add_argument(
        "--factor-results",
        type=Path,
        default=THIS_DIR / "output" / "factor_result_rows.jsonl",
