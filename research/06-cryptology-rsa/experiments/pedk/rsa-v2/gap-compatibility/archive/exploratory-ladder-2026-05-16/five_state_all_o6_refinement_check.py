#!/usr/bin/env python3
"""Check the five-state all-o6 PEDK refinement on a fresh band."""
...
DEFAULT_OUTPUT_DIR = THIS_DIR / "output" / "five_state_all_o6_refinement_check"
RULE_ID = "pedk_five_state_all_o6_refinement_check_v1"
SOURCE_RULE_ID = "pedk_all_o6_candidate_rule_check_v1"
DEFAULT_MIN_FACTOR = 2201
DEFAULT_MAX_FACTOR = 2600
...
def all_o6_signature(rows: list[dict[str, object]]) -> str:
    """Return the all-o6 excluded factor-neighborhood signature."""
    matches = [
        row for row in rows
        if row["candidate_rule"] == NARROW_RULE_NAME
        and "excluded_factor_neighborhood_signature" in row
    ]
    signatures = {str(row["excluded_factor_neighborhood_signature"]) for row in matches}
    if len(signatures) != 1:
        raise ValueError("expected exactly one all-o6 signature")
    return next(iter(signatures))


def refinement_states(summary: dict[str, object]) -> set[str]:
    """Return the survived five-state refinement from the fourth-band check."""
    states = {str(state) for state in summary["survived_public_phase_states"]}
    if len(states) != 5:
        raise ValueError("expected exactly five survived public phase states")
    return states
...
def check_refinement(
    ...
    """Return refinement check rows for one fresh band."""
    support = Counter(
    ...
    candidate_rows = [
        {
            ...
            "candidate_rule": "five_state_all_o6_refinement",
            "candidate_status": (
                "survived_fresh_band"
            ...
            "falsification_criterion": (
                "A row falsifies this candidate if S(N) is one of the five "
                "surviving public phase states and F(p,q) is the all-o6 "
    ...
    summary = {
        "rule_id": RULE_ID,
        "source_rule_id": SOURCE_RULE_ID,
        "status": "measured_refined_candidate_rule_check",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Check the five-state all-o6 PEDK refinement on a fresh band."
    )
    ...
    parser.add_argument("--candidate-rows", type=Path, default=CANDIDATE_PATH)
...
def main(argv: list[str] | None = None) -> int:
    """Run the five-state all-o6 refinement check."""
    ...
    excluded_signature = all_o6_signature(read_jsonl(args.candidate_rows))
    ...
    candidate_rows, falsification_rows, state_support_rows, summary = check_refinement(
        states,
        excluded_signature,
        forward_rows,
    )
