#!/usr/bin/env python3
"""Grok Round 13 mirror for the lane 163|19 next-parity residue lift."""
...
ROUND12_OUTPUT_DIR = THIS_DIR / "output" / "grok_round12_next_parity_obstruction"
OUTPUT_DIR = THIS_DIR / "output" / "grok_round13_next_parity_residue_lift"
RULE_ID = "pedk_grok_round13_next_parity_residue_lift_v1"
TARGET_LANE = "163|19"


def load_micro_rows() -> list[dict[str, object]]:
    """Return Grok Round 12 prior-surface micro rows."""
...
def next_parity_definition() -> dict[str, object]:
    """Return the source-level next-parity definition."""
    return {
        "rule_id": RULE_ID,
        ...
        "definition": (
            "gap_grammar selects the first minimum divisor-count interior "
            "value. Its exact_type_key suffix is the carrier_family of that "
            "winner_value. For d=4 winners, winner_value % 2 == 0 gives "
            "d4_even and winner_value % 2 == 1 gives d4_odd."
        ),
...
def symbolic_parity_table() -> list[dict[str, object]]:
    """Return the two offset-parity classes for following-left residue 43."""
...
def falsifier_contract() -> dict[str, object]:
    """Return the falsifier contract."""
    return {
        "rule_id": RULE_ID,
        "component_law": "next_parity_odd",
        "target_lane": TARGET_LANE,
        "falsifier_contract": (
            "Any valid row on the 163|19 prior surface with even "
            "next_winner_offset, equivalently next_parity=odd because the "
            "following-left endpoint is odd, invalidates this component law."
        ),
...
def summary() -> dict[str, object]:
    ...
    return {
        ...
        "measured_even_lift_count": sum(
            1 for row in lift_rows if row["next_parity_by_lift"] == "even"
        ),
        "measured_odd_lift_count": sum(
            1 for row in lift_rows if row["next_parity_by_lift"] == "odd"
        ),
...
def main() -> int:
    """Run the Grok Round 13 residue-lift mirror."""
