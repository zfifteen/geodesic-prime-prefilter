#!/usr/bin/env python3
"""Round 8 finite obstruction workbench for the lane-survival selector."""
...
OUTPUT_DIR = THIS_DIR / "output" / "codex_round8_finite_obstruction_table"
RULE_ID = "pedk_codex_round8_finite_obstruction_table_v1"
SIGNATURE_EQUATIONS = {
    "13|19": {
...
def lane_coordinates(lane: dict[str, object]) -> dict[str, object]:
    ...
    if (a_mod6 - b_mod6) % 6 != expected_difference:
        raise ValueError(f"bad same-phase lane coordinates: {lane['lane']}")
    return {
        "a_mod6": a_mod6,
        "b_mod6": b_mod6,
...
def measured_surviving_rows() -> list[dict[str, object]]:
    """Return measured same-phase Round 7 target rows."""
    return [
        row
        for row in target_rows(annotated_rows())
        if row["same_mod36"]
    ]
...
def finite_lane_obstruction_table() -> list[dict[str, object]]:
    """Return the 12-lane finite candidate obstruction table."""
    measured_map = measured_lane_map()
    table = []
    for lane in theoretical_same_phase_lanes():
        coordinates = lane_coordinates(lane)
        table.append(obstruction_for_lane(lane, coordinates, measured_map))
    return table


def terminal_image_table() -> list[dict[str, object]]:
    """Return terminal image rows for the measured survivor lanes."""
    rows = []
    for row in measured_surviving_rows():
...
def proof_obligations() -> dict[str, object]:
    """Return the remaining proof obligations after Round 8."""
    return {
        "rule_id": RULE_ID,
        "proof_status": "incomplete",
        "obligations": [
            {
                "name": "CRT core reduction",
                "status": "prior_measured_scaffold",
                "claim": "Rres=o4|o4 reduces the target to factor-core orientations 13|19 and 19|13.",
            },
            {
                "name": "same-phase lane table",
                "status": "finite_algebra_table_emitted",
                "claim": "same_mod36 expands those orientations into exactly 12 ordered mod-180 lanes.",
            },
            {
            ...
            },
            {
                ...
                "claim": "The two selected lanes land in lower predecessor pairs 19|22 and 29|18 with four slots.",
            },
        ],
    }
...
def summary() -> dict[str, object]:
    ...
    return {
        "rule_id": RULE_ID,
        "status": "measured_candidate_obstruction_workbench",
        "theorem_status": "hypothesis_not_proved",
...
def main() -> int:
    """Run the finite obstruction workbench."""
