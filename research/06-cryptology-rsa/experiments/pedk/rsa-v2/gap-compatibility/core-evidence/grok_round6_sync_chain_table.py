#!/usr/bin/env python3
"""Grok Round 6 dependency table for same-phase synchronization."""
...
OUTPUT_DIR = THIS_DIR / "output" / "grok_round6_sync_chain_table"
RULE_ID = "grok_round6_sync_chain_table_v1"
...
def theoretical_same_phase_lanes() -> list[dict[str, object]]:
    """Return the 12 ordered mod-180 same-phase lanes for factor cores 13|19."""
    lanes = []
    for p_mod30, q_mod30 in ((13, 19), (19, 13)):
        orientation = f"{p_mod30}|{q_mod30}"
        for p_mod180 in range(p_mod30, 180, 30):
            q_matches = [
                q_mod180
                for q_mod180 in range(q_mod30, 180, 30)
                if q_mod180 % 36 == p_mod180 % 36
            ]
            if len(q_matches) != 1:
                raise ValueError(f"expected one q phase match for {p_mod180}")
            q_mod180 = q_matches[0]
            lanes.append(
                {
                    "rule_id": RULE_ID,
...
def compact_row(row: dict[str, object]) -> dict[str, object]:
    """Return dependency-relevant row fields."""
    return {
        "rule_id": RULE_ID,
        "window": row["window"],
        "case_id": row["case_id"],
...
def dependency_key(row: dict[str, object]) -> str:
    """Return compact truth-state key for the synchronization chain."""
    return "|".join(
        (
...
def summary(rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    accidental_lifts = [
        row
        for row in non_same_phase_targets
        if row["lower_terminal_four_slot"]
    ]
    ...
    return {
        ...
        "same_phase_target_lanes": sorted(
        ...
        "theoretical_same_phase_lane_count": len(theoretical_same_phase_lanes()),
        "observed_same_phase_lane_count": len(
            {str(row["factor_mod180_lane"]) for row in same_phase_targets}
        ),
...
def main() -> int:
    """Run the Grok Round 6 dependency table."""
    rows = annotated_rows()
    targets = target_rows(rows)
    accidental_lifts = [
        row
    ...
    write_jsonl(
        ...
        [compact_row(row) for row in targets],
    )
    write_jsonl(
        OUTPUT_DIR / "theoretical_same_phase_lanes.jsonl",
        theoretical_same_phase_lanes(),
    )
    write_jsonl(
        OUTPUT_DIR / "accidental_lift_rows.jsonl",
        [compact_row(row) for row in accidental_lifts],
    )
