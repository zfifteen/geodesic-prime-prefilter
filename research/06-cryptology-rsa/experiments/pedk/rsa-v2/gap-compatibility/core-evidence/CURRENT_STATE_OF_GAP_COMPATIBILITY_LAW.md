...
active_surface = public_at_winner -> right_following_endpoint_residue_class
candidate_law = clean right-following classes exclude exact endpoint-pair cells
```
...
For the two factor endpoints together, the clean classes currently under
inspection are:
...
The clean classes have not falsified:
...
family from neighboring right-following families that do falsify.
...
public_at_winner
    ...
    determines admissible right-following endpoint residue classes
```
