#!/usr/bin/env python3
"""Grok co-pilot mirror for Round 19: Shadow-surface compression test.

This is the independent Grok implementation of the shadow-surface compression
matrix per the Round 19 design contract (shadow rows = public_prefix + any
failure in the five post-prefix gates).

Contract fidelity:
- Shadow predicate is lane-conditioned (same_mod36 + factor_mod180_lane +
  rres_o4_o4 + at_winner) but then any post-prefix gate failure counts.
- Gate evaluation uses *computed* transport (width/residue → prev_open,
  right + winner_offset → next_parity) exactly as in Round 17/18.
- derive_shadow_row_class is a strict first-failure mechanical rule (order
  matches the original pipeline).
- Two surfaces are reported: all shadow rows vs exact-one-gate-only.
- Explicit tests for symmetric-pair collapse, survivor contamination, and
  exact-one-gate behavior.
- All status flags remain hypothesis_not_proved.

Grok auditor additions:
- Explicit lane_blind_boundary reminder (the experiment remains lane-selected).
- grok_mirror_note in the final summary for traceability.
- No changes to classification logic that would alter measured counts.

Output location: output/grok_round19_shadow_surface_compression/
"""
...
OUTPUT_DIR = THIS_DIR / "output" / "grok_round19_shadow_surface_compression"
RULE_ID = "pedk_grok_round19_shadow_surface_compression_v1"
...
def enriched_state(
    ...
    """Compute width/residue transport and parity lift (identical to Round 18/19)."""
...
def public_prefix(row: dict[str, object]) -> bool:
    """The common public shadow prefix (lane-agnostic part of the base surface)."""
...
def gate_vector(row: dict[str, object], state: dict[str, object]) -> dict[str, bool]:
    """Five post-prefix gates evaluated on *computed* transport values."""
    return {
...
def derive_shadow_row_class(gates: dict[str, bool]) -> tuple[str, str]:
    """Fixed first-failure classification (mechanical, pre-specified)."""
...
def shadow_row_for_lane(
    ...
    """Return enriched shadow row or None."""
    ...
    return {
        ...
        "lower_predecessor_open_slot_count": int(row["lower_predecessor_open_slot_count"]),
...
def shadow_rows() -> list[dict[str, object]]:
    """All shadow rows (lane-conditioned public prefix + any post-prefix failure)."""
    rows = annotated_rows()
...
def shadow_derivation_rules() -> list[dict[str, object]]:
    return [
        {"class": "entry_width_residue_open_offset_shadow_defect", "rule": "computed_prev_open_offset != 4"},
        {"class": "entry_d_bound_shadow_defect", "rule": "computed_prev_open_offset=4 and prev_d>4"},
        {"class": "directed_tuple_shadow_defect", "rule": "entry gates pass and directed_tuple not in allowed set"},
        {"class": "exit_d_bound_shadow_defect", "rule": "allowed tuple passes and next_d>4"},
        {"class": "exit_offset_parity_shadow_defect", "rule": "allowed tuple and next_d pass but computed_next_parity != odd"},
    ]
...
def compression_summary(
    ...
    pair_class = profile_by_lane[SYMMETRIC_PAIR[0]]["primary_shadow_mechanism_class"]
    survivor_classes = {
    ...
    exact_pair_collapsed = exact_pair_classes[SYMMETRIC_PAIR[0]] == exact_pair_classes[SYMMETRIC_PAIR[1]]
    ...
    return {
        ...
        "symmetric_pair_collapsed_into_entry_width_residue": (
            pair_collapsed and pair_class == "entry_width_residue_open_offset_shadow_defect"
        ),
        ...
        "grok_mirror_note": (
            "Grok co-pilot independent mirror. Classification rules, enriched_state, "
            "and gate_vector logic are identical to Codex Round 19. Result must match."
        ),
    }
...
def main() -> int:
    ...
    write_jsonl(OUTPUT_DIR / "exact_one_gate_lane_shadow_profiles.jsonl", exact_one_profiles)
