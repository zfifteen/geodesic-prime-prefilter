#!/usr/bin/env python3
"""Round 18 mechanism-compression matrix for the same-phase lane selector."""
...
OUTPUT_DIR = THIS_DIR / "output" / "codex_round18_component_obstruction_compression"
RULE_ID = "pedk_codex_round18_component_obstruction_compression_v1"
...
def rows_for_lane_at_stage(
    ...
    """Return the cascade row and rows at the last non-zero stage for a lane."""
    current = rows
...
def unique_sorted(values: list[object]) -> list[object]:
    """Return deterministic unique values."""
...
def summarize_rows(
    ...
    """Return aggregate mechanism features for representative rows."""
...
def a_b_mod6(lane: dict[str, object]) -> dict[str, int]:
    """Return phase coordinates in p = p_mod30 + 30a and q = q_mod30 + 30b."""
    return {
...
def mechanism_features(
    ...
    """Return the feature vector used to derive the mechanism class."""
...
def derive_mechanism_class(features: dict[str, object]) -> tuple[str, str]:
    """Return mechanism class and explicit rule from public features."""
...
def factor_relevance(row: dict[str, object]) -> str:
    """Return the current operational factor-relevance classification."""
    if row["survivor_status"] == "survivor" and row["terminal_image_status"] == (
...
def matrix_rows() -> list[dict[str, object]]:
    """Return the 12-lane mechanism-compression matrix."""
    rows = annotated_rows()
...
def falsifier_contract(mechanism_class: str, lane: str) -> str:
    """Return a falsifier contract for a derived mechanism class."""
...
def mechanism_groups(rows: list[dict[str, object]]) -> dict[str, object]:
    """Return excluded and survivor rows grouped by derived mechanism class."""
...
def compression_summary(rows: list[dict[str, object]]) -> dict[str, object]:
    """Return the Round 18 compression summary."""
    ...
    return {
        ...
        "candidate_common_invariant": (
            ...
            if mechanism_law_count_after >= component_law_count_before
...
def proposed_next_proof_object(rows: list[dict[str, object]]) -> dict[str, object]:
    """Return the next proof object selected by the compression decision rule."""
    ...
    return {
        ...
        "recommended_next_law": next(
            (
                row["derived_mechanism_class"]
                for row in rows
                if row["derived_mechanism_class"]
                == "entry_width_residue_open_offset_mismatch"
            ),
            rows[0]["derived_mechanism_class"],
        ),
        "theorem_status": "hypothesis_not_proved",
        "universal_proof_complete": False,
    }
...
def main() -> int:
    """Run the component-obstruction compression matrix."""
    rows = matrix_rows()
    if len(rows) != 12:
