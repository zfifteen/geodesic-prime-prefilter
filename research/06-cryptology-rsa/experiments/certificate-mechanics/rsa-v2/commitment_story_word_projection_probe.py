...
GRAMMAR_DIR = EXPERIMENTS_DIR / "grammar-evidence" / "rsa-v2"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=GRAMMAR_DIR / "output" / "grammar_recursive_solved_surface" / "recursive_target_rows.jsonl",
        help="Existing solved recursive target rows.",
    )
    parser.add_argument(
        "--fresh-recursive-rows",
        type=Path,
        default=GRAMMAR_DIR / "output" / "fresh_rsa_challenge_recursive_surface" / "recursive_target_rows.jsonl",
        help="Fresh RSA challenge recursive target rows.",
    )
    parser.add_argument(
        "--expanded-recursive-rows",
        type=Path,
        default=GRAMMAR_DIR / "output" / "grammar_recursive_target_catalog" / "recursive_target_rows.jsonl",
        help="Expanded recursive target catalog rows.",
    )
    parser.add_argument(
        "--solved-inverse-rows",
        type=Path,
        default=GRAMMAR_DIR / "output" / "grammar_inverse_word_exclusion" / "inverse_word_rows.jsonl",
        help="Existing solved inverse word rows.",
    )
    parser.add_argument(
        "--fresh-inverse-rows",
        type=Path,
        default=GRAMMAR_DIR / "output" / "fresh_rsa_challenge_inverse_word_exclusion" / "inverse_word_rows.jsonl",
