...
`research/06-cryptology-rsa/scripts/scaleup_corpus.json` as the active target surface:
...
p=Path('research/06-cryptology-rsa/scripts/scaleup_corpus.json')
...
from pathlib import Path
p=Path('research/06-cryptology-rsa/scripts/scaleup_corpus.json')
out=Path('research/06-cryptology-rsa/output/geofac_scaleup/competition_256_challenge_ratio_probe_summary.json')
data=json.loads(p.read_text())
rows=[]
for bits_text, cases in data.items():
    bits=int(bits_text)
...
```sh
python3 research/06-cryptology-rsa/scripts/pgs_256_center_prior_probe.py --scale-bits 256 --rung 1 --min-center-bits 96 --max-center-bits 108
```

```sh
python3 research/06-cryptology-rsa/scripts/pgs_256_center_prior_probe.py --scale-bits 256 --rung 1 --min-center-bits 96 --max-center-bits 108 --route-only
```
...
- exact per-scale sets are written to
  `research/06-cryptology-rsa/output/geofac_scaleup/competition_256_challenge_ratio_probe_summary.json`
...
- changed `research/06-cryptology-rsa/scripts/pgs_geofac_scaleup.py`
...
edit research/06-cryptology-rsa/scripts/pgs_geofac_scaleup.py to change 0.40 to 0.3994 in _family_center_estimate for challenge_like
```
...
- `research/06-cryptology-rsa/scripts/pgs_geofac_scaleup.py`: updated center calculation for challenge_like
...
```sh
python3 research/06-cryptology-rsa/scripts/pgs_geofac_scaleup.py --scale-bits 256 --rung 1 --cases 1 --seed 0 --router-mode audited_family_prior
```
...
`research/06-cryptology-rsa/scripts/build_scaleup_corpus.py`.
...
for p in (ROOT/'src'/'python', ROOT/'benchmarks'/'python'/'predictor'):
    if str(p) not in sys.path:
        sys.path.insert(0,str(p))
import pgs_geofac_scaleup as base
row=next(row for row in json.loads((ROOT/'research/06-cryptology-rsa/scripts/scaleup_corpus.json').read_text())['256'] if row['family']!='challenge_like')
...
```sh
python3 -m py_compile research/06-cryptology-rsa/scripts/pgs_geofac_scaleup.py
pytest research/06-cryptology-rsa/tests/test_pgs_geofac_scaleup.py::test_big_int_prime_traversal_outputs_unique_primes_at_4096_scale -q
pytest research/06-cryptology-rsa/tests/test_pgs_geofac_scaleup.py::test_pure_pgs_router_is_deterministic_except_timing -q
```
...
- `research/06-cryptology-rsa/output/geofac_scaleup/pgs_256_balanced_1_centered_interleaved_r1_result.json`
...
- `research/06-cryptology-rsa/output/geofac_scaleup/pgs_s256_balanced_2_centered_interleaved_r1_result.json`
- `research/06-cryptology-rsa/output/geofac_scaleup/pgs_s256_balanced_2_centered_window_order_r1_result.json`
...
- `research/06-cryptology-rsa/output/geofac_scaleup/pgs_s256_balanced_1_centered_window_order_r1_result.json`
