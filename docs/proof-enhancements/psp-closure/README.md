...
Close the modulus-link / tiling collision step in `PROOF.md` (Prime-Square Proximity Theorem, ~lines 604 to 625) so the bound
...
| Step | Lemma | Status |
|------|-------|--------|
| 0 | Square-branch setup (τ(w)=3, earlier τ≥4, r≤p) | **PROVED**: `PROOF.md` lines 587 to 602 |
| 1 | Row to Factor Assignment | **PROVED** (defs in PROOF.md + psp-closure) |
...
## Lemma 1: Row to Factor Assignment `PROVED` (see PROOF.md rigorous edit + defs)

**Statement.** Under square-branch hypotheses, each `m ∈ {1,…,M}` admits a unique admissible placement `(ℓ_m, h_m, d_m)`.

**Proof obligations:**

- [x] Existence from compositeness + root-straddling form (`PROOF.md` 596 to 600)
...
**Reference:** `lean-4/PGS/ChamberReset.lean` theorem `near_root_exclusion_bound` (lines 280 to 346).

**PROOF.md action:** Replace informal derivation at lines 612 to 616 with named cross-reference only.
...
**Assembly:** Assume `r² − p > C(q)` ⇒ all rows `m ≤ M` composite ⇒ Row to Factor Assignment ⇒ partition symmetric / M-rough ⇒ Injectivity + Pigeonhole ⇒ contradiction.
...
1. Insert `### Row to Factor Assignment` after line 602
2. Insert `### Injectivity on M-Rough Rows` and `### Modulus-Link Pigeonhole Bound`
3. Replace lines 618 to 619 (informal density) with Lemmas 4a to 4c
...
| Artifact | Role |
|----------|------|
| `PROOF.md` lines 575 to 627 | Live PSP proof (patch target) |
| `lean-4/PGS/ChamberReset.lean` | `MRough`, `near_root_exclusion_bound`, `prime_square_proximity_theorem` |
| `docs/square_branch_theorem_plan.md` | Prior execution plan (Phases 1 to 4) |
| Discussion #23 comment 17562285 | Row to Factor outline |
