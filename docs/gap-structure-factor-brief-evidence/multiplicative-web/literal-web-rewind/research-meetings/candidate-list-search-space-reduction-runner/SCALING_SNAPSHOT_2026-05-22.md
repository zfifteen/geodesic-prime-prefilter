...
- On real derived public motifs at ~60 bits the current rule set (81 rules, PG-001 to PG-081) delivers **41.16%** average hypothesis-space reduction.
- On a real-derivation ladder (48 to 72 bits, 15 samples/level) average reduction declines gradually from **38.4%** (48 bits) to **27.9%** (72 bits).
...
The grammar lever is real, measurable, and still improving with targeted rule addition from larger enriched surfaces (27k to 34k bands). We are not yet at the point of diminishing returns for the current approach.

## Key Measured Results

### 1. Real-Derivation Mid-Scale Probe (58 to 66 bit semiprimes, 10 samples, final rule set)

- Average reduction: **44.95%** (end of current mining cycle)
- Best cases (well-covered `o2_d4_a2_d4_odd@mid` variants): 71 to 74%
- Typical exotic high-a cases (a7 to a20+): 52 to 60%
- Remaining stubborn cases (specific a1/a2/a3 with bad phases): ~2%

### 2. Real-Derivation Scaling Ladder (48 to 72 bits, 15 samples per level)

| Bit Length | Avg Reduction | Std Dev | Range     | Unresolved |
|------------|---------------|---------|-----------|------------|
| 48         | 38.42%        | ±24.8%  | 0 to 72%     | 0          |
| 56         | 35.89%        | ±29.7%  | 0 to 69%     | 0          |
| 64         | 34.11%        | ±20.0%  | 0 to 73%     | 0          |
| 72         | 27.91%        | ±26.8%  | 0 to 73%     | 0          |
...
Each batch was derived directly from patterns observed in the 27k to 34k enriched multiplication-map surfaces.

## Remaining Coverage Gaps (from 64 to 74 bit diagnostic)
...
These families are well-represented in the 32k to 34k enriched maps but were only lightly covered by the original 601_5500 rules.

## Interpretation for the RSA Goal

1. **The public grammar lever is scale-relevant.** Even with current coverage gaps we are deleting 25 to 41% of the factor-neighborhood hypothesis space using only public structural information on real 60 to 72 bit semiprimes.
...
4. **Next branch point** (to be decided after one more short mining round):
   - Continue mining the 27k to 35k family for the remaining stubborn motifs (likely to push mid-scale average into the mid-40s or low-50s).
   - Freeze the current rule set and build infrastructure (efficient motif derivation + candidate generation) to test at 80 to 100+ bits and eventually the Wikipedia RSA numbers.
...
All numbers above are reproducible with the checked-in code and the public 27k to 34k enriched map files.
