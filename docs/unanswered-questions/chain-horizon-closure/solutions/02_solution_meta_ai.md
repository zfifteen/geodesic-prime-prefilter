...
- Proposes six concrete horizon laws H0 to H6 and a strict acceptance gate (100% closure of false nodes, $H/\sqrt{n}$ tiny, no audit labels).
...
2. For every chain processed at $10^{12}$, $10^{15}$, $10^{18}$:
    ...
    - compute H0 to H6 as defined in answer 05
3. Acceptance gate:
...
Run the probe, publish `least_factor_frontier.csv`. Based on the simulation and the repo's zero-failure audit surface, I expect H0 or H1 will pass the gate, converting the 56 to 58% bridge to pure PGS.
