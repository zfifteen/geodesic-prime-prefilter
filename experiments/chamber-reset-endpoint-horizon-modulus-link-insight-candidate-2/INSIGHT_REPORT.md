...
| Chain-horizon closure | Operational, ~56 to 58% at 10¹⁵/10¹⁸ | Documented in chain-horizon-closure answers |
...
These are emitted identically in Python (`simple_pgs_generator.py:78 to 155`) and C (`pgs_chamber.c:587 to 628`).
...
## Comparison to existing horizon candidates (answers 05 to 06)
...
| H2 to H6 | residue/LCM/row-level fits | Higher complexity; pilot suggests unnecessary at first order |
...
If the full shadow-chain mining experiment confirms the pilot, the 56 to 58% non-PGS high-scale bridge converts to pure PGS by a single horizon substitution. The theorem target is:
