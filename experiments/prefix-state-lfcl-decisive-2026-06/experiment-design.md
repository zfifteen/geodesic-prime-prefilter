...
- Pre-registered closure laws **L1 to L4** plus harness control **L0**.
...
| **L0 to L4** | Tested forward closure laws | **Forbidden** |
| **V** | Post-hoc: `τ(r)=2`, `r == q_ref` | After selection only |
| **D1** | Uniqueness counter on admissible sets | No `q_ref` in set definition |

**Lane isolation:** Laws L0 to L4 and D1 must not import `q_ref`, `gap`, or any
...
resolved-survivor classification in L1 to L4.

### Allowed vs forbidden divisor-count use

| Use | L1 to L4 closure laws |
| --- | --- |
...
**L1 to L4 expectations (pre-registered, not verdicts):**
...
| Verdict | Rule |
| --- | --- |
| **Substantively supported (finite surface)** | At least one law L1 to L4 passes substantive support row above |
| **Substantively falsified** | Any law fires early with `r ≠ q_ref`, OR D1 shows declared unique closure while `U(B) > 1` |
| **Unresolved (default if no early support)** | L0 fails harness; L1 to L4 all falsified-as-predictor (no early fire) or only post-hoc match; no mismatch |
...
experiments/prefix-state-lfcl-decisive-2026-06/
  ...
  closure_laws.py              # L0 to L4 implementations
...
2. **`closure_laws.py`**: L0 to L4 first-fire scanners; no `q_ref` imports.
3. **`semantic_audit.py`**: extend prior `audit_utils` patterns.
4. **`test_prefix_closure.py`**: L0 fails on `p=73`; L1 to L4 do not import `q_ref`; SA clean; synthetic prefix fixture with known mismatch/silent gap.
...
If all L1 to L4 show `early_fire_count = 0` on R2:
